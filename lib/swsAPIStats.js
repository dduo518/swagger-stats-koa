...
swsAPIStats.prototype.countResponse = function(ctx) {
    ...
    this.promClientMetrics.api_request_duration_milliseconds.labels(ctx.method, ctx.sws.api_path, ctx.statusCode).observe(ctx.sws.duration);
    this.promClientMetrics.api_request_size_bytes.labels(ctx.method, ctx.sws.api_path, ctx.statusCode).observe(ctx.sws.req_clength);
    this.promClientMetrics.api_response_size_bytes.labels(ctx.method, ctx.sws.api_path, ctx.statusCode).observe(ctx.sws.res_clength);
