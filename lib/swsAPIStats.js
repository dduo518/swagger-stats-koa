...
swsAPIStats.prototype.countRequest = function(ctx) {
    ...
    if (('match' in ctx.sws) && ctx.sws.match) {
        ...
        this.countParametersStats(ctx.sws.api_path, ctx.method, ctx);
    }
...
swsAPIStats.prototype.countResponse = function(ctx) {
    ...
    apiOpStats.countResponse(ctx.response.status, codeclass, ctx.sws.duration, ctx.sws.res_clength);
    ...
    if (!(ctx.response.status in apiOpDetails.code)) {
        apiOpDetails.code[ctx.response.status] = { count: 0 };
    }

    apiOpDetails.code[ctx.response.status].count++;
    ...
    this.promClientMetrics.api_request_total.labels(ctx.method, ctx.sws.api_path, ctx.response.status).inc();

    this.promClientMetrics.api_request_duration_milliseconds.labels(ctx.method, ctx.sws.api_path, ctx.response.status).observe(ctx.sws.duration);
    this.promClientMetrics.api_request_size_bytes.labels(ctx.method, ctx.sws.api_path, ctx.response.status).observe(ctx.sws.req_clength);
    this.promClientMetrics.api_response_size_bytes.labels(ctx.method, ctx.sws.api_path, ctx.response.status).observe(ctx.sws.res_clength);
