...
swsCoreStats.prototype.countResponse = function(ctx) {
    ...
    if ("_contentLength" in ctx && ctx['_contentLength'] !== null) {
        ...
        // Try header
        if (ctx.response.headers['content-length'] !== null) {
            resContentLength = parseInt(ctx.response.headers['content-length']);
        }
    }
    ...
    this.all.countResponse(ctx.response.status, codeclass, duration, resContentLength);

    // Update method-specific stats
    var method = req.method;
    if (method in this.method) {
        var mstat = this.method[method];
        mstat.countResponse(ctx.response.status, codeclass, duration, resContentLength);
    }
